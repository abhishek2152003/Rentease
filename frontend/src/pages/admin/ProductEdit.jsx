...
const ProductEdit = () => {
    ...
    useEffect(() => {
        if (product) {
            ...
            setPriceMonthly(product.priceMonthly);
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 bg-white p-8 rounded-lg shadow-sm border border-zinc-200">
            ...
            {loadingUpdate && <div className="text-zinc-500">Updating...</div>}

            {isLoading ? <div className="text-zinc-500">Loading...</div> : error ? (
                <div className="text-red-500">{error.error}</div>
            ) : (
                <form onSubmit={submitHandler} className="space-y-6">
                    <div>
                        <label className="block text-sm font-medium text-zinc-700">Name</label>
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 block w-full rounded-md border-zinc-300 py-2 px-3 text-zinc-900 border focus:ring-zinc-900 focus:border-zinc-900" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 block w-full rounded-md border-zinc-300 py-2 px-3 text-zinc-900 border focus:ring-zinc-900 focus:border-zinc-900" />
                        </div>
                        <div>
                    ...
                    <button type="submit" disabled={loadingUpdate} className="bg-zinc-900 text-white px-6 py-2 rounded-md font-medium hover:bg-zinc-800 transition-colors">
                        Update Details
                    </button>
                </form>
            )}
